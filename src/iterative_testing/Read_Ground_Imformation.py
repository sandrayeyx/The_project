...
def max_distance(ElevationAngle, orbital_height):
    ElevationAngle = math.radians(ElevationAngle)
    earth_radius = 6371
    distance = math.sqrt(
        orbital_height ** 2 + 2 * orbital_height * earth_radius + (math.cos(ElevationAngle) * earth_radius) ** 2
        ) - math.cos(ElevationAngle) * earth_radius
    return distance


def to_h3_index(lat, lon, resolution=0):
    # return h3.geo_to_h3(lat, lon, resolution)
    # 在新版 h3-py 中，geo_to_h3 已被替换为 latlng_to_cell
...
def get_h3_neighbors(h3_index):
    ...
    # 在 H3 v4.0+ 中，k_ring 函数已被重命名为 grid_disk
    return h3.grid_disk(h3_index, 1)


def get_connections_h3(ground_users, satellites, ElevationAngle):
    connections = {}
    cell_satellites = {}
    ...
    max_dist = max_distance(ElevationAngle, satellites[next(iter(satellites))]["altitude"])
